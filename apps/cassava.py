...
data = pd.read_csv('data/mhogo.csv')
colors = ['#D1B835','#D00FD6','#904949','#380E38','#212521']
fig = go.Figure()

fig.add_trace(go.Indicator(
    mode = "number+delta",
    value = ar[ar['Item'] == 'Cassava'].Y2018.sum(),
    ...
    delta = {'reference': ar[ar['Item'] == 'Cassava'].Y2017.sum(), 'relative': True},
    domain = {'x': [0, 0.5], 'y': [0.5, 0]}))

fig.add_trace(go.Indicator(
    mode = "number+delta",
    value = pdd[pdd['Item'] == 'Cassava'].Y2018.sum(),
    ...
    delta = {'reference': pdd[pdd['Item'] == 'Cassava'].Y2017.sum(), 'relative': True},
    domain = {'x': [0.5, 1], 'y': [0, 1]}))

fig.add_trace(go.Indicator(
    mode = "number+delta",
    value = yy[yy['Item'] == 'Cassava'].Y2018.sum() / 10000,
    ...
    delta = {'reference': yy[yy['Item'] == 'Cassava'].Y2017.sum()/10000, 'relative': True},
    domain = {'x': [0.5, 0], 'y': [1, 0.5]}))

fig1 = country_info('Somalia', item='Cassava').iplot(asFigure = True, kind = 'bar', barmode = 'overlay', x = 'Years', y = 'Area harvested',
                                              yTitle = 'Years', xTitle = 'Area Harvested (Hectares)', theme = 'polar', colorscale = 'prgn', orientation = 'h')

fig2 = country_info('Somalia', item='Cassava').iplot(asFigure = True, kind = 'scatter', mode = 'lines', x = 'Years', y = 'Production',
                                              xTitle = 'Years', yTitle = 'Tonnes', theme = 'polar',subplots = True,subplot_titles = True,colors = '#AD1A31', title = 'Cassava Production in Somalia from 2014 - 2018')

fig3 = px.scatter_mapbox(total_element('Cassava','production'), lat='Lat',lon='Lon',color='Total',hover_name='Area',size = 'Total',
                        hover_data={'Lat':False, 'Lon':False}, labels = {'Area':'Element'}, color_continuous_scale=colors,
                        zoom=1.4, width=1900, height=700, template='presentation',center=None, mapbox_style='light',title='Global Cassava Production in Tonnes(2014-2018)')

fig4 = data[data['Element'] == 'Production'].iplot(asFigure = True, kind = 'pie', labels = 'Area', values = 'Y2018', legend = False,textinfo = 'label+percent',theme = 'polar', hole = .6, linecolor = 'white', colors = colors, linewidth = .5, title = 'Cassava Production in Africa as at 2018')
fig5 = country_info('Somalia', item='Cassava').iplot(asFigure = True, kind = 'scatter', mode = 'lines+markers', x = 'Years', y = 'Yield',subplots = True,subplot_titles = True,xTitle = 'Years', yTitle = 'Tonne per Hectare', title = 'Cassava Yield for Somalia (2014 - 2018)', colorscale = 'puor', theme = 'polar', interpolation = 'spline')
fig6 = country_info('Somalia', item='Cassava').iplot(asFigure = True, kind = 'barh', x = 'Years', y = 'Area harvested', yTitle = 'Years', subplots = True,subplot_titles = True,xTitle = 'in Hectares', title = 'Area Harvested in Somalia', theme = 'polar', colorscale = 'piyg')

layout = html.Div([
    dbc.Container([
        dbc.Row([
                dbc.Col(html.H1('World Cassava Statistics as at 2018', className='text-center'), className='mb-4 mt-5')
            ]),
        dbc.Row(
            dbc.Col(html.Div([
