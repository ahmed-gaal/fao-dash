...
fig.add_trace(go.Indicator(
    ...
    delta={'reference': yy[yy['Item'] == 'Bananas'].Y2017.sum()/10000,
            'relative': True},
...
def bar_graph(x=None, y=None, xTitle=None, yTitle=None, title=None, how=None,
              colors=None):
    df = country_info('Somalia', item='Bananas')
...
fig3 = px.scatter_mapbox(total_element('production', 'Bananas'), lat='Lat',
...
layout = html.Div([
    dbc.Container([
        dbc.Row([
                dbc.Col(html.H1('World Bananas Statistics as at 2018',
                                className='text-center'), className='mb-4 mt-5')
            ]),
        dbc.Row(
            dbc.Col(html.Div([
                    dcc.Graph(id='Chart',
        ...
        dbc.Row([
            dbc.Col(
                html.H2(children='Banana Production in Somalia.', style={
        ...
        dbc.Row([
            dbc.Col(
                html.H2(children='Banana Yield in Somalia.', style={
                'font-family':'Overpass, sans-serif', 'font-size':'200%', 'font-weight':'bold'
            }),
                width={'size':6, 'offset':4}
            )
